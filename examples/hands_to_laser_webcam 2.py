...
class HandToLaserController:
    ...
    def generate_laser_frame(self) -> List[bytes]:
        ...
        if self.projection_mode == ProjectionMode.POINT:
            # Single point mode
            num_points = max(10, power_12bit // 200)
            ...
            # Drawing mode with trail
            if len(self.drawing_trail) > 1:
                for i, (x, y) in enumerate(self.drawing_trail):
                    # Fade older points
                    fade_factor = (i + 1) / len(self.drawing_trail)
    ...
    def process_hands(self, hand_landmarks_list: List[List[Tuple[int, int]]], 
        ...
        if self.projection_mode == ProjectionMode.POINT:
            ...
            for i, landmark_id in enumerate(key_landmarks):
